...
async function scrape() {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let headline = headlineMatch ? headlineMatch[1].trim() : `Position ${i+1}`;
    
    // Skip non-position blocks (e.g. Pros/Cons which don't have images)
    ...
    if (imgMatch) {
      imageUrl = imgMatch[1];
      const filename = `lauvette_${positions.length + 1}.jpg`;
